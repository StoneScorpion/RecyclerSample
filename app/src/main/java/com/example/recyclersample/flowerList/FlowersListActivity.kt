    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, intentData: Intent?) {
        ...
        if (requestCode == newFlowerActivityRequestCode && resultCode == Activity.RESULT_OK) {
            intentData?.let { data ->
                ...
                val flowerTamanio = data.getStringExtra(FLOWER_TAMANIO)
                val flowerTipo = data.getStringExtra(FLOWER_TIPO)
                val flowerCientifico = data.getStringExtra(FLOWER_CIENTIFICO)
                val flowerColor = data.getStringExtra(FLOWER_COLOR)

                if (flowerTipo != null) {
                    if (flowerCientifico != null) {
                        if (flowerColor != null) {
                            flowersListViewModel.insertFlower(flowerName, flowerDescription, flowerTamanio, flowerTipo, flowerCientifico, flowerColor)
                        }
                    }
                }
            }
